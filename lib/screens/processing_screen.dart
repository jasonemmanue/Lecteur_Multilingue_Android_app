...
class _ProcessingScreenState extends ConsumerState<ProcessingScreen>
  ...
  void _startPolling() {
    _fetchStatus();
    _pollingTimer = Timer.periodic(AppConfig.pollingInterval, (_) => _fetchStatus());
  }
  ...
  Future<void> _fetchStatus() async {
    try {
      ...
      ref.read(queueProvider.notifier).updateJob(widget.jobId, job);

      if (job.status == JobStatus.done && job.outputUrl != null) {
        _pollingTimer?.cancel();
        await _handleSuccess(job);
      } else if (job.status == JobStatus.error) {
        _pollingTimer?.cancel();
        await _handleError(job.errorMessage ?? 'Erreur inconnue');
      }
    } on ApiException catch (e) {
      _consecutiveErrors++;
      if (_consecutiveErrors >= AppConfig.maxPollingRetries) {
        _pollingTimer?.cancel();
        setState(() => _errorMsg = e.userMessage);
      }
    } catch (_) {
      _consecutiveErrors++;
    }
  ...
  Future<void> _handleSuccess(TranslationJob job) async {
    setState(() { _isDownloading = true; _downloadProgress = 0; });

    try {
      final localPath = await _downloadService.downloadTranslatedVideo(
        downloadUrl: '${AppConfig.baseUrl}/download/${widget.jobId}',
        jobId:       widget.jobId,
      ...
      // Notification push
      await NotificationService.notifyTranslationDone(
        videoTitle:     widget.videoTitle ?? 'Vidéo',
        targetLanguage: widget.targetLanguage ?? '',
      );

      // Retirer de la file
      ref.read(queueProvider.notifier).dequeue(widget.jobId);

      if (mounted) {
        context.pushReplacement(AppRoutes.player, extra: localPath);
      }
    } on ApiException catch (e) {
      setState(() {
  ...
  Future<void> _handleError(String message) async {
    await NotificationService.notifyTranslationError(
      videoTitle:   widget.videoTitle ?? 'Vidéo',
  ...
  Widget build(BuildContext context) {
    ...
    final isError  = _job?.status == JobStatus.error || _errorMsg != null;
    ...
    return Scaffold(
      ...
      body: _isDownloading
          ? _DownloadingIndicator(progress: _downloadProgress)
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 20),
          const Text('Téléchargement de la vidéo traduite…',
